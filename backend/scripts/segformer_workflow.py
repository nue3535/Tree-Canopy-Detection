"""SegFormer aligned with tree_canopy_multimodel_training_evaluation_fixed.ipynb:
nvidia/segformer-b2-finetuned-ade-512-512, num_labels=3; 512, batch 2, 50 epochs, AdamW 1e-4 / wd 1e-4, DiceCELoss, multimodel aug."""
...
def _compute_test_accuracy(
    ...
    try:
        ...
        if not records:
            return None
        loader = DataLoader(
            SegFormerDataset(records, config.img_size, augment_style=config.augment_style),
...
def _predict_mask(
    ...
    tensor = _seg_preproc_transform(img_size, imagenet_norm=imagenet_norm)(image_rgb).unsqueeze(0).to(device)
    with torch.no_grad():
...
def _save_evaluation_artifacts(
    ...
    for idx, record in enumerate(records, start=1):
        ...
        pred_mask = _predict_mask(
            model, image_bgr, config.img_size, device, imagenet_norm=config.augment_style == "multimodel",
        )
...
def train(config: TrainConfig) -> None:
    ...
    try:
        processor = SegformerImageProcessor.from_pretrained(SEGFORMER_PRETRAINED_ID)
    except Exception:
    ...
    training_results = {
        ...
        "config_note": f"Aligned with multimodel notebook: {SEGFORMER_PRETRAINED_ID}, 512, batch 2, 50 epochs, DiceCELoss, multimodel Albumentations.",
    }
